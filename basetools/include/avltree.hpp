...
private:
    AVLNode* RemoveNode(AVLNode* node, AVLNode* parent = nullptr) noexcept
    {
        ...
        // zwei Kinder: Vorg�nger aus linkem Teilbaum herausl�sen und relinken
        node = UnlinkNode(node->left, node);          // liefert neue Wurzel an dieser Stelle
        if (m_info.heightHasChanged) 
            node = BalanceLeftShrink(node);
        DeleteNode(m_info.workingNode);               // urspr�nglichen Knoten entfernen
        return node;
#else
        if (not node->right) {
            m_info.heightHasChanged = true;
        ...
        // zwei Kinder: Schl�ssel/Daten mit Vorg�nger tauschen, dann Vorg�nger l�schen
        node->left = UnlinkNode(node->left);
        if (m_info.heightHasChanged) 
            node = BalanceLeftShrink(node);
        DeleteNode(m_info.workingNode);               // der im Unlink gel�ste Vorg�nger
        return node;
#endif
    }
