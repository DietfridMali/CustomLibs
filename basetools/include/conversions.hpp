...
namespace Conversions
{
 ...
 noexcept
    ...
    static float Stretch(float v, float vMin, float vMax) {
...
#ifdef _DEBUG
        v = (v < m) ? m - m * (m - v) / (m - vMin) : m + (1.0f - m) * (v - m) / (vMax - m);
        if (v < 0.0f)
            return 0.0f;
        if (v > 1.0f)
            return 1.0f;
        return v;
#else
        return (v < m) ? m - m * (m - v) / (m - vMin) : m + (1.0f - m) * (v - m) / (vMax - m);
#endif
    }
 ...
 noexcept
    ...
    // Liefert floor(sqrt(i)) f�r beliebige Integraltypen (signed/unsigned)
    ...
    constexpr std::make_unsigned_t<Int> IntSqrt(Int i) {
        ...
        // Startwert: 2^ceil(bit_width(n)/2)  (leichtes �ber-Estimate)
        ...
        // Exakte Korrektur (max. 1�2 Schritte), gesch�tzt vor Overflow via Division
        while (x > n / x)
            --x;
        while ((x + 1) <= n / (x + 1))
            ++x;
 ...
 noexcept
    {
        ...
        while ((d > 0) and (m % d != 0)) {   // Schritt zur�ck, bis echter Teiler
            --d;
        }
        ...
        if constexpr (std::signed_integral<Int>) {
            if (n < 0)
                w = -w;           // bei negativen n a negativ w�hlen
        }
