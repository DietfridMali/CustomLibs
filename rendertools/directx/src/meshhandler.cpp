...
void MeshHandler::Recycle(void) noexcept {
    // Reclaim the slot that is about to be reused this frame. CommandListHandler::BeginFrame has
    // already waited this slot's frame fence, so its parked meshes are GPU-idle.
    MeshTable& usedList = m_usedLists[commandListHandler.FrameIndex()];
    for (Mesh* mesh : usedList)
        m_freeList.Push(mesh);
    usedList.Clear();
    m_frameNumber = commandListHandler.FrameNumber();
}


Mesh* MeshHandler::AllocMesh(uint32_t meshBufferMask) noexcept {
    if (m_frameNumber != commandListHandler.FrameNumber())
        Recycle();
    ...
    if (not mesh)
        mesh = new Mesh();
    mesh->ResetGfxData();
    m_usedLists[commandListHandler.FrameIndex()].Push(mesh);
    return mesh;
}
