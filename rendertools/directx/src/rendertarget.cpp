...
bool RenderTarget::SelectDrawBuffers(int bufferIndex, eDrawBufferGroups drawBufferGroup)
{
    ...
    const D3D12_CPU_DESCRIPTOR_HANDLE* pDSV = (m_depthBufferIndex >= 0 and m_bufferInfo[m_depthBufferIndex].m_dsvHandle.IsValid()) ? &m_bufferInfo[m_depthBufferIndex].m_dsvHandle.cpu : nullptr;
    if (count > 0)
...
void RenderTarget::Clear(int bufferIndex, eDrawBufferGroups /*drawBufferGroup*/, bool clear)
{
    ...
    if (bufferIndex < 0) {
        for (int i = 0; i < m_colorBufferCount; ++i)
            if (m_bufferInfo[i].m_rtvHandle.IsValid())
                list->ClearRenderTargetView(m_bufferInfo[i].m_rtvHandle.cpu, zero, 0, nullptr);
    }
    else if ((bufferIndex < m_colorBufferCount) and m_bufferInfo[bufferIndex].m_rtvHandle.IsValid()) {
        list->ClearRenderTargetView(m_bufferInfo[bufferIndex].m_rtvHandle.cpu, zero, 0, nullptr);
    }
    if ((m_depthBufferIndex >= 0) and m_bufferInfo[m_depthBufferIndex].m_dsvHandle.IsValid())
...
Texture* RenderTarget::GetRenderTexture(const RTRenderParams& params, int /*tmuIndex*/)
{
    ...
    BufferInfo& info = m_bufferInfo[src];
    if (not info.m_resource)
        return nullptr;
...
bool RenderTarget::RenderAsTexture(Texture* source, const RTRenderParams& params, const RGBAColor& color)
{
    if (params.destination < 0) {
        ...
        if (not EnableBuffers(params.destination, dbSingle, params.clearBuffer, true))
            return false;
