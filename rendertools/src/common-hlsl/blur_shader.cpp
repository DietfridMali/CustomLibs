...
//
// TWO SHADERS, one per distance source: "bilateralBlur" reads the world positions, "bilateralBlurDepth"
// the scene depth. Each declares only the texture it reads. A shader that declares both and picks one
// at run time leaves the other slot to whatever was bound there last, and Vulkan validates every
// declared slot - a stale texture of another view type, or a depth buffer that is a writable
...
static const String& BilateralBlurFS(bool useSceneDepth) {
    ...
    static const String positionSource(R"(
            ...
            float3 SurfaceKey(float2 uv) {
                return uWorldPositions.SampleLevel(s2, uv, 0).xyz;
            }
            float SurfaceDistance(float3 keyC, float3 keyS, float3 NC) {
                return abs(dot(keyS - keyC, NC));
            }
    )");
    ...
    static const String depthSource(R"(
            ...
            float3 SurfaceKey(float2 uv) {
                return float3(EyeDepth(uSceneDepth.SampleLevel(s3, uv, 0).r), 0.0, 0.0);
            }
            float SurfaceDistance(float3 keyC, float3 keyS, float3 NC) {
                return abs(keyS.x - keyC.x);
            }
    )");
    ...
    static const String body(R"(
            float4 PSMain(PSInput i) : SV_Target {
                ...
                [loop] for (int k = -radius; k <= radius; ++k) {
                    ...
                    float dP = SurfaceDistance(keyC, SurfaceKey(suv), NC);
