...
// pDynamicOffsets[]. A frame that needs more than the slot's buffer holds chains further buffers
// on the spot (FrameData::overflow), so an allocation only fails when the device is out of memory.
// Reset() at frame start rewinds the cursor and drops the chained buffers; if the previous frame
// overshot the capacity, the slot's own buffer is reallocated up to kMaxCap.
//
// Usage:
