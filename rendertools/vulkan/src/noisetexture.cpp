...
// =================================================================================================
// Vulkan-specific Deploy + SetParams overrides. All compute / mipmap / file I/O code lives in the
// common base_noisetexture.cpp; this unit only configures the VK sampler state and routes Deploy
// through the platform-neutral Upload<2D,3D>Texture helpers in vkupload.

// -------------------------------------------------------------------------------------------------
// Common sampler setup helper. The original per-class SetParams variants only differ in min/mag
// filter mode; everything else (repeat wrap, comparator, anisotropy) is identical.
...
bool BlueNoiseTexture::Deploy(int) {
    return Upload3DTexture(*this, m_gridSize.x, m_gridSize.y, m_gridSize.z, GfxPixelFormat::R8_UNorm, reinterpret_cast<const void*>(m_data.Data()));
}
