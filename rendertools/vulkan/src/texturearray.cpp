...
// -------------------------------------------------------------------------------------------------
// One copy per (slot, mip). The chain is built here rather than by the driver because Vulkan has no
...
bool GfxTextureArray::UpdateSlot(int slotIndex) {
    ...
    return UploadTextureArrayData(m_image, m_layoutTracker, &slot, 1,
