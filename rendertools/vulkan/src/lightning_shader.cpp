...
//
// Lightning ribbon shader (Vulkan). Mirrors directx/src/lightning_shader.cpp verbatim -- HLSL compiled to
// SPIR-V via DXC, and the register() bindings (u0 segments, b0 FrameConstants, b1 ShaderConstants, t2
// sceneDepth) map through the VK bind-register tables exactly as on DX. The projection matrix already
// carries the Vulkan NDC convention (no in-shader Y flip, like ParticleDrawVS), and the manual depth
// occlusion uses the same window-space z compare (i.pos.z > sceneNdcZ) as the OIT particle pass.
// Each polyline segment is one instance; the VS pulls it from the structured buffer and expands the unit
// quad into a screen-space ribbon along the miter (bisector) of the adjacent segments in view space, so
// neighbouring segments that share a node emit the SAME edge and the strip stays gap-/overlap-free. The
// FS draws a white core with a cool-blue halo, additively into the dedicated glow buffer (HDR -> bloom),
// and discards fragments behind the opaque scene (the glow buffer has no depth attachment of its own).
...
PSInput VSMain(VSInput v) {
    ...
    // FS, which dims colour AND coverage by it -> the line turns continuous, not brighter. abs() on the
    // projection element: the VK projection may carry a negative Y scale (NDC convention).
    float widthComp = 1.0;
    if (minWidthPx > 0.0) {
...
// =================================================================================================
// Impact flare (Vulkan). Byte-identical HLSL to the DX flare (compiled to SPIR-V via DXC); the register()
// bindings map through the VK bind-register tables exactly as on DX. A round, view-aligned billboard at each
// bolt endpoint, instanced from the LightningFlare buffer; the FS depth-tests against the scene depth like the
// ribbon FS (the soft smoothstep fade stays disabled, occ = 1), the VS nudge onto the front face keeps the
// flare visible where it hits.
...
PSInput VSMain(VSInput v) {
    ...
    vc.xy += v.pos.xy * (f.width * flareScale) / float2(1.0, flareAspect);   // view-aligned billboard; flareAspect > 1 -> wide/flat ellipse
