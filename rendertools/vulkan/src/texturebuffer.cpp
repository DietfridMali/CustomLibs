...
// Kernel-Caching: ein Eintrag pro Radius 0..15, L�nge 31 (Mitte = Index 15)
...
void TextureBuffer::GaussBlur(uint16_t strength, bool wrap) {
    ...
    if (m_info.m_componentCount == 3) {
        AutoArray<RGB8> blurBuffer(m_info.m_width * m_info.m_height);
        GaussBlurH<RGB8>(blurBuffer.Data(), reinterpret_cast<RGB8*>(m_data.Data()), m_info.m_width, m_info.m_height, r, wrap);
        GaussBlurV<RGB8>(reinterpret_cast<RGB8*>(m_data.Data()), blurBuffer.Data(), m_info.m_width, m_info.m_height, r, wrap);
    }
    else if (m_info.m_componentCount == 4) {
        AutoArray<RGBA8> blurBuffer(m_info.m_width * m_info.m_height);
        GaussBlurH<RGBA8>(blurBuffer.Data(), reinterpret_cast<RGBA8*>(m_data.Data()), m_info.m_width, m_info.m_height, r, wrap);
        GaussBlurV<RGBA8>(reinterpret_cast<RGBA8*>(m_data.Data()), blurBuffer.Data(), m_info.m_width, m_info.m_height, r, wrap);
    }
}
