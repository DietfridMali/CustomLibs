...
void TextRenderer::Render(String text, eTextAlignments alignment, int flipVertically, int renderAreaWidth, int renderAreaHeight, bool useRenderTarget) {
    if (m_font and (text.Length() > 0)) {
        if (not useRenderTarget)
            ...
            RenderTarget* renderTarget = GetRenderTarget(2);
            if (renderTarget != nullptr) {
                RenderToBuffer(text, alignment, renderTarget, baseRenderer.GetViewport(), renderAreaWidth, renderAreaHeight);
                RenderToScreen(renderTarget, flipVertically); // render outline to viewport
            }
        }
    }
}
