...
bool UDPSocket::SendBroadcast(const uint8_t* data, int len, bool subnetOnly) {
    // Empfänger bestimmen
    NetworkEndpoint dst = subnetOnly ? DirectedBroadcast(GetPort()) : NetworkEndpoint::LimitedBroadcast(GetPort());
